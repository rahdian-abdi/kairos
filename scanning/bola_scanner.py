...
def check_bola_vulnerability(method, url, headers, body, uuid_file=None):
    ...
    # If a UUID file is provided, read the UUIDs from the file
    ...
    if path_param_match:
        ...
        if re.match(r'^[a-zA-Z0-9_-]*:[0-9a-fA-F-]{36}$', id_in_path) or re.match(r'^[0-9a-fA-F-]{36}$', id_in_path):  # Check if the ID is a (prefixed) UUID
            ...
            # If UUIDs to test are provided, use them; otherwise, generate random UUIDs
    ...
    # Check for ID in the query parameters
    if '?' in url:
        ...
        if param_match:
            ...
            if re.match(r'^[0-9a-fA-F-]{36}$', param_value):  # Check if the ID is a UUID
